...
export default function ExploreSkills() {
  ...
  // Limit to 6 items (3 rows on desktop grid) unless showAll is true
  const displayedSkills = showAll ? filteredSkills : filteredSkills.slice(0, 6);

  return (
    <div className="bg-white min-h-screen text-zinc-900">
      {/* HERO & SEARCH HEADER */}
      <section className="bg-black text-white pt-20 sm:pt-28 pb-12 sm:pb-16 px-4 sm:px-6 relative overflow-hidden">
        <div className="absolute top-1/2 left-1/4 -translate-y-1/2 w-64 h-64 sm:w-96 sm:h-96 bg-blue-600/20 blur-[100px] sm:blur-[130px] rounded-full pointer-events-none" />
        <div className="absolute top-1/2 right-1/4 -translate-y-1/2 w-64 h-64 sm:w-96 sm:h-96 bg-purple-600/15 blur-[100px] sm:blur-[130px] rounded-full pointer-events-none" />

        <div className="max-w-6xl mx-auto relative z-10 text-center space-y-4 sm:space-y-6">
          <div className="inline-flex items-center gap-2 px-3 py-1 sm:px-3.5 sm:py-1.5 rounded-full border border-white/10 bg-white/5 backdrop-blur-md text-[11px] sm:text-xs font-mono text-purple-300">
            <GraduationCap size={14} className="text-blue-600" />
            <span className="text-blue-400">Discover Verified Peer Swaps</span>
          </div>
          ...
          <p className="text-zinc-400 text-xs sm:text-lg max-w-2xl mx-auto leading-relaxed">
          ...
          <div className="pt-2 sm:pt-4 max-w-2xl mx-auto">
            <div className="relative flex items-center">
              <Search
                size={18}
                className="absolute left-4 sm:left-5 text-zinc-400 pointer-events-none"
              />
              <input
                type="text"
                value={searchQuery}
                ...
                className="w-full pl-11 sm:pl-14 pr-4 sm:pr-6 py-3 sm:py-4 rounded-xl sm:rounded-2xl bg-zinc-900/90 border border-zinc-800 text-white placeholder:text-zinc-500 text-xs sm:text-base focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all shadow-2xl backdrop-blur-md"
              />
            </div>
          </div>
        </div>
      ...
      {/* CATEGORY FILTER BAR */}
      <section className="bg-white border-b border-zinc-200 py-3 sm:py-5 px-4 sm:px-6 sticky top-0 z-30 backdrop-blur-md bg-white/90">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 sm:gap-3 overflow-x-auto no-scrollbar py-1 flex-1">
            <span className="text-[10px] sm:text-xs font-bold text-zinc-400 uppercase tracking-wider flex items-center gap-1 shrink-0 pr-1">
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    ...
                    setShowAll(false); // reset limit on category switch
                  }}
                  className={`inline-flex items-center gap-1.5 px-3 py-1.5 sm:px-4 sm:py-2 rounded-xl text-xs font-semibold transition-all duration-200 shrink-0 ${
                    isActive
      ...
      {/* SKILLS GRID */}
      <section className="bg-zinc-50/50 pt-8 pb-16 px-4 sm:px-6">
        <div className="max-w-7xl mx-auto">
          {filteredSkills.length === 0 ? (
            <div className="text-center py-16 sm:py-20 bg-white rounded-3xl border border-zinc-200 shadow-sm px-4">
                ...
                <AnimatePresence>
                  {displayedSkills.map((item) => (
                    <motion.div
                      ...
                      className="bg-white rounded-2xl sm:rounded-3xl border border-zinc-200 p-4 sm:p-6 flex flex-col justify-between hover:border-blue-400 hover:shadow-xl hover:shadow-blue-500/5 transition-all duration-300 group"
                    >
                      <div>
                        {/* User Header */}
                        <div className="flex items-center justify-between gap-2 mb-4">
                          <div className="flex items-center gap-2.5">
                            <img
                              src={item.avatar}
                              alt={item.user}
                              className="w-8 h-8 sm:w-10 sm:h-10 rounded-full object-cover border border-zinc-200"
                            />
                            <div>
                          ...
                          <span className="text-[9px] sm:text-[10px] font-mono font-bold px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full bg-zinc-100 text-zinc-600 border border-zinc-200 uppercase shrink-0">
                            {item.level}
                          </span>
                        </div>

                        {/* Title */}
                        <h3 className="text-base sm:text-lg font-bold text-zinc-950 group-hover:text-blue-600 transition-colors mb-3">
                          {item.title}
                        </h3>

                        {/* Offers/Wants */}
                        <div className="space-y-2 bg-zinc-50/80 p-3 sm:p-3.5 rounded-xl sm:rounded-2xl border border-zinc-100 mb-4">
                          <div className="flex items-start gap-1.5 text-xs">
                            <span className="font-semibold text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded text-[10px] sm:text-xs shrink-0">
                              Offers:
                            </span>
                            <span className="text-zinc-700 font-medium line-clamp-1 text-[11px] sm:text-xs">
                              {item.offers}
                            </span>
                          </div>
                          <div className="flex items-start gap-1.5 text-xs">
                            <span className="font-semibold text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded text-[10px] sm:text-xs shrink-0">
                              Wants:
                            </span>
                            <span className="text-zinc-700 font-medium line-clamp-1 text-[11px] sm:text-xs">
                              {item.wants}
                      ...
                      <div className="pt-3 border-t border-zinc-100 flex items-center justify-between">
                        ...
                        <Link
                          ...
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 sm:px-4 sm:py-2 rounded-xl bg-zinc-900 text-white hover:bg-blue-600 font-semibold text-xs transition-colors duration-200 shrink-0"
                        >
                          <span className="hidden sm:inline">Initiate Swap</span>
              ...
              {filteredSkills.length > 6 && (
                <div className="mt-12 text-center">
                  <button
                    ...
                    className="inline-flex items-center gap-2 px-8 py-3.5 rounded-full bg-zinc-900 hover:bg-blue-600 text-white font-bold text-xs sm:text-sm transition-all duration-300 shadow-md hover:shadow-lg active:scale-95"
                  >
                    <span>
                      {showAll
                        ? "Show Less"
                        : `View All (${filteredSkills.length - 6} More)`}
                    </span>
                    <ChevronDown
                      size={16}
      ...
      {/* CALL TO ACTION */}
      <section className="bg-black text-white py-16 sm:py-20 px-4 sm:px-6 border-t border-zinc-800 text-center">
        <div className="max-w-3xl mx-auto space-y-4 sm:space-y-5">
          ...
          <div className="pt-2">
            <Link
              to="/dashboard/explore"
              className="inline-block px-6 sm:px-8 py-3 sm:py-3.5 rounded-full bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs sm:text-sm transition-all shadow-lg shadow-blue-600/30"
            >
